...
const optionBaseClass =
  'flex w-full cursor-pointer items-center justify-between gap-3 px-4 py-2 text-sm text-[var(--system-black)] transition hover:bg-[var(--docker-blue)]/10 focus:bg-[var(--docker-blue)]/15 focus:outline-none dark:text-[var(--grey-text)] dark:hover:bg-[var(--docker-blue)]/20 dark:focus:bg-[var(--docker-blue)]/25';
...
const Select: React.FC<SelectProps> = ({
  ...
  const renderOption = (option: SelectOption, index: number) => {
    ...
    return (
      <button
        ...
        className={`${optionBaseClass} ${isDisabled ? optionDisabledClass : ''} ${
          isSelected ? 'bg-[var(--docker-blue)]/10 dark:bg-[var(--docker-blue)]/20' : ''
        }`}
      >
        <span className="flex flex-col items-start text-left">
          <span className="font-medium">{option.label}</span>
          {option.description ? (
            <span className="text-xs text-[var(--medium-gray)] dark:text-[var(--grey-text)]/70">
              {option.description}
            </span>
          ) : null}
        </span>
  ...
  const selectedLabelClass = selectedOption
    ? ''
    : 'text-[var(--medium-gray)] dark:text-[var(--grey-text)]/70';
  ...
  return (
    <div ref={containerRef} className={containerClass}>
      ...
      >
        <span className={`flex flex-col text-left ${selectedLabelClass}`}>
          ...
          {selectedOption && selectedOption.description ? (
            <span className="text-xs font-normal text-[var(--medium-gray)] dark:text-[var(--grey-text)]/70">
      ...
      {open ? (
        ...
        >
          {options.length === 0 ? (
            <div className="px-4 py-3 text-sm text-[var(--medium-gray)] dark:text-[var(--grey-text)]/70">
              {noOptionsMessage}
            </div>
          ) : (
